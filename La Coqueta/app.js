document.addEventListener('DOMContentLoaded', () => {
    ...
    const dashboardViewContent = document.getElementById('dashboard-view-content');
    ...
    // --- LÓGICA DE NAVEGACIÓN POR PESTAÑAS (BI/DSS) ---
    tabBi.addEventListener('click', () => {
        tabBi.classList.add('active', 'text-pink-600');
        tabBi.classList.remove('text-gray-500');
        tabDss.classList.remove('active', 'text-pink-600');
        tabDss.classList.add('text-gray-500');
        dashboardViewContent.classList.remove('hidden');
        matrixView.classList.add('hidden');
    });
    tabDss.addEventListener('click', () => {
        tabDss.classList.add('active', 'text-pink-600');
        tabDss.classList.remove('text-gray-500');
        tabBi.classList.remove('active', 'text-pink-600');
        tabBi.classList.add('text-gray-500');
        matrixView.classList.remove('hidden');
        dashboardViewContent.classList.add('hidden');
    });

    // --- LÓGICA DE LA MATRIZ DE DECISIÓN (DSS) ---
    const calculateMatrix = () => {
        const weights = { c1: 0.40, c2: 0.30, c3: 0.20, c4: 0.10 };
        const getScore = (option, criterion) => parseFloat(document.getElementById(`score-${option}-${criterion}`).value) || 0;
        ...
        const scores = [
            { name: 'Opción A: Marcas Nacionales', score: scoreA },
            { name: 'Opción B: Modelo Híbrido', score: scoreB },
            { name: 'Opción C: Marcas Importadas', score: scoreC }
        ];
    ...
    matrixInputs.forEach(input => input.addEventListener('input', calculateMatrix));
    ...
    const renderEstiloChart = (responses) => {
        ...
        const counts = (responses.flatMap(r => r.estilo).filter(Boolean)).reduce((acc, v) => { const label = labels[v] || v; acc[label] = (acc[label] || 0) + 1; return acc; }, {});
        renderChart('estilo-chart', 'bar', { labels: Object.keys(counts), datasets: [{ data: Object.values(counts), backgroundColor: ['#ec4899', '#be185d', '#a21caf', '#7c3aed', '#5b21b6', '#f472b6'], borderRadius: 6 }] }, { indexAxis: 'y', responsive: true, plugins: { legend: { display: false } } });
    };
    const renderPrecioChart = (responses) => {
        const labels = { '<30': 'Hasta $30', '30-60': '$31 - $60', '>60': 'Más de $60' };
        const counts = responses.reduce((acc, r) => { const label = labels[r.precio] || 'No responde'; acc[label] = (acc[label] || 0) + 1; return acc; }, {});
        renderChart('precio-chart', 'bar', { labels: Object.keys(counts), datasets: [{ label: 'Número de Usuarias', data: Object.values(counts), backgroundColor: ['#ec4899', '#be185d', '#a21caf'], borderRadius: 8 }] }, { responsive: true, plugins: { legend: { display: false } } });
    };
    ...
    // --- LÓGICA DEL SORTEO, ADMIN Y ACCIONES RÁPIDAS ---
